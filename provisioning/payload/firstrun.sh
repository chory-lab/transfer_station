...
# Both forms: bootstrap writes PI_PASSWORD_B64, a hand-edited config.env
# writes PI_PASSWORD. Base64 is encoding, not encryption, so the encoded one
# must be scrubbed just as thoroughly.
sed -i 's/^PI_PASSWORD=.*/PI_PASSWORD=<redacted>/; s/^PI_PASSWORD_B64=.*/PI_PASSWORD_B64=<redacted>/'     "$PAYLOAD/config.env"
